...
\begin{exercice}
Soit $f \in L(E,F)$ où $E$ et $F$ sont deux espaces vectoriels normés sur $\mathbb{K}$.
...
\begin{exercice}
Vérifier que $O(2) = \{A \in M_2 (\R), A^T \cdot A = I_2\}$, l'ensemble des matrices orthogonales, est un compact de $GL_{2}(\mathbb{R})$.
\end{exercice}
...
\begin{exercice}
Soit $E$ un espace vectoriel normé. On rappelle que le diamètre d'une partie $A$ bornée non vide de $E$ est le réel $\text{diam}(A) = \sup \{ \|x-y\|, (x,y) \in A^{2} \}$.
...
\begin{exercice}
Soit $(E, N)$ un espace vectoriel normé et $(u_{n})_{n \in \mathbb{N}}$ une suite convergente de limite $\ell$.
...
\begin{correction}{13}
    ...
    $f(x,y) = \sqrt{\Sigma_{i = 1}^n x_i^2}\sqrt{\Sigma_{i = 1}^n y_n} - | \Sigma_{i=1}^n x_iy_i|$ est continue par théorèmes d'opérations car s'exprime continûment en fonction des coordonnées.  
...
$\varphi{(0)} = 0$.
Par la linéarité de la dérivation il est clair que $\varphi (\lambda f) = \lambda \varphi f$, de même $\varphi (f + g) = \varphi{(f)} + \varphi{(g)}$. Donc $\varphi$ est linéaire. \\
...
Donc $\forall \epsilon > 0, \exists \delta > 0, \forall x,y \in [0, 2T], |x-y| \le \delta \implies |f(x)-f(y)| \le \epsilon$. \\
...
L'union de deux intervalles sur lesquels $f$ est uniformément continue (avec recouvrement) conserve l'uniforme continuité (prouvé en exemple du cours).
Donc $f$ est uniformément continue sur $\mathbb{R}^{+}$.
\end{correction}
